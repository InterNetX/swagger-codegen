...
public class DartClientCodegen extends DefaultCodegen implements CodegenConfig {
	...
	public DartClientCodegen() {
		...
		languageSpecificPrimitives = new HashSet<String>(
				Arrays.asList("String", "bool", "int", "num", "double", "DateTime", "List", "Object", "Map"));
		...
		typeMapping.put("HkIndustryTypeConstants", "String");
	...
	public void processOpts() {
		...
		supportingFiles.add(new SupportingFile("models_lib.mustache", "", "models_lib.dart"));
	}
	...
	public String toVarName(String name) {
		...
		name = name.replaceAll("-", "_"); // FIXME: a parameter should not be assigned. Also declare the methods
											// parameters as 'final'.
	...
	public String toModelName(String name) {
		...
		if(isReservedWord(name)) {
			LOGGER.warn(
				name + " (reserved word) cannot be used as model filename. Renamed to " + camelize("model_" + name));
	...
	public void postProcessModelProperty(CodegenModel model, CodegenProperty property) {
		LOGGER.info("Check if " + property.name + " is primitive");
		if(!property.isPrimitiveType) {
			if(!property.isDateTime && !property.isDate) {
				LOGGER.info(property.name + " is not primitive and not datetime, add import");
				LOGGER.info("Check if " + property.complexType + " matches " + model.classname);
				if(!property.complexType.equals(model.classname)) {
					model.imports.add(property.complexType);
				} else {
					LOGGER.info("Import is same as class");
					model.imports.remove(property.complexType);
				}
			}
		}
	...
	private boolean buildEnumFromVendorExtension(CodegenModel cm) {
		if(!cm.isEnum || cm.allowableValues == null || !useEnumExtension
